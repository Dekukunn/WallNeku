  ...
  Future<void> _fetchWallpapers() async {
    try {
      final fetchedWallpapers =
          await apiService.fetchWallpapers('Cars'); // Filter 'Animals'
      if (!mounted) return; // Check if the widget is still mounted
      setState(() {
        carsWallpapers = fetchedWallpapers;
        isLoading = false;
      });
    } catch (e) {
      print('Error: $e');
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.grey[850],
           title: Text('Cars Wallpaper',style: TextStyle(color: Colors.white),),
           leading: IconButton(
                  ...
                  child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return WallpaperTile1(
                          imageUrl: carsWallpapers[index],
                          title: 'Wallpaper ${index + 1}',
                          isFavorite: favoritesProvider.isFavorite(carsWallpapers[
                              index]), // Check if wallpaper is favorite
                          onFavoriteToggle: () =>
                              favoritesProvider.toggleFavorite(carsWallpapers[
